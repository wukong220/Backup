...
FixStyle(rodrigid,FixRodrigid)

#else

#ifndef LMP_FIX_RODRIGID_H
#define LMP_FIX_RODRIGID_H

#include "fix.h"

namespace LAMMPS_NS {

class FixRodrigid : public Fix {
 public:
  FixRodrigid(class LAMMPS *, int, char **);
  virtual ~FixRodrigid();
  virtual int setmask();
  virtual void init();
 ...
 protected:
  ...
  double tr_start,tr_stop,tr_target;
...
E: Rodrigid fix must come before NPT/NPH fix
