...
FixRodrigid::FixRodrigid(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  int i,ibody;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"force") == 0) {
      ...
      if (iarg+8 > narg) error->all(FLERR,"Illegal fix rigid command");
      ...
      tr_start = force->numeric(FLERR,arg[iarg+4]);
      tr_stop = force->numeric(FLERR,arg[iarg+5]);
      tr_period = force->numeric(FLERR,arg[iarg+6]);
      seed = force->inumeric(FLERR,arg[iarg+7]);
      
      if (t_period <= 0.0 || tr_period)
...
void FixRodrigid::setup_pre_neighbor()
{
  pre_neighbor();
}
...
void FixRodrigid::setup(int vflag)
{
  int i,n,ibody;
...
void FixRodrigid::initial_integrate(int vflag)
{
  double dtfm;
...
/* ----------------------------------------------------------------------
   ...
   只作用在质心上，平动去除了质心drag力,只有热运动
    转动，单独设置 damp 
------------------------------------------------------------------------- */

void FixRodrigid::post_force(int vflag)
{
  if (me == 0) {
    ...
    tr_target = tr_start + delta * (tr_stop-tr_start);
    double tsqrt = sqrt(t_target);
    double trsqrt = sqrt(tr_target);
    ...
    for (int i = 0; i < nbody; i++) {
      gamma1 = 0.0; //-masstotal[i] / t_period / ftm2v;
      gamma2 = sqrt(masstotal[i]) * tsqrt *
        sqrt(24.0*boltz/t_period/dt/mvv2e) / ftm2v;
        
      langextra[i][0] =  gamma2*(random->uniform()-0.5);
      langextra[i][1] =  gamma2*(random->uniform()-0.5);
      langextra[i][2] =  gamma2*(random->uniform()-0.5);

      gamma1 = -1.0 / tr_period / ftm2v;
      gamma2 = trsqrt * sqrt(24.0*boltz/tr_period/dt/mvv2e) / ftm2v;
...
void FixRodrigid::final_integrate()
{
  int i,ibody;
  double dtfm;
...
void FixRodrigid::initial_integrate_respa(int vflag, int ilevel, int iloop)
{
...
void FixRodrigid::final_integrate_respa(int ilevel, int iloop)
{
  dtf = 0.5 * step_respa[ilevel] * force->ftm2v;
  final_integrate();
}
...
int FixRodrigid::dof(int tgroup)
{
...
void FixRodrigid::deform(int flag)
{
  if (flag == 0)
    for (int ibody = 0; ibody < nbody; ibody++)
...
void FixRodrigid::setup_bodies_static()
{
  int i,ibody;
...
void FixRodrigid::setup_bodies_dynamic()
{
  int i,ibody;
...
void FixRodrigid::readfile(int which, double *vec, 
...
void FixRodrigid::write_restart_file(char *file)
{
  if (me) return;
...
double FixRodrigid::memory_usage()
{
  int nmax = atom->nmax;
...
void FixRodrigid::grow_arrays(int nmax)
{
...
void FixRodrigid::copy_arrays(int i, int j, int delflag)
{
  body[j] = body[i];
...
void FixRodrigid::set_arrays(int i)
{
  body[i] = -1;
  xcmimage[i] = 0;
  displace[i][0] = 0.0;
...
int FixRodrigid::pack_exchange(int i, double *buf)
{
  buf[0] = ubuf(body[i]).d;
...
int FixRodrigid::unpack_exchange(int nlocal, double *buf)
{
...
void FixRodrigid::zero_momentum()
{
  for (int ibody = 0; ibody < nbody; ibody++)
    vcm[ibody][0] = vcm[ibody][1] = vcm[ibody][2] = 0.0;
...
void FixRodrigid::zero_rotation()
{
  for (int ibody = 0; ibody < nbody; ibody++) {
    angmom[ibody][0] = angmom[ibody][1] = angmom[ibody][2] = 0.0;
...
double FixRodrigid::compute_scalar()
{
...
void *FixRodrigid::extract(const char *str, int &dim)
{
...
double FixRodrigid::extract_erotational()
{
  double wbody[3],rot[3][3];
...
/* ----------------------------------------------------------------------
   ...
   18 values per body
   xcm = 0,1,2; vcm = 3,4,5; fcm = 6,7,8; torque = 9,10,11; omega = 12 13 14 image = 15,16,17  
------------------------------------------------------------------------- */

double FixRodrigid::compute_array(int i, int j)
{
  if (j < 3) return xcm[i][j];
  if (j < 6) return vcm[i][j-3];
  if (j < 9) return fcm[i][j-6];
  if (j < 12) return torque[i][j-9];
  if (j < 15) return omega[i][j-12];
  if (j == 15) return (imagebody[i] & IMGMASK) - IMGMAX;
  if (j == 16) return (imagebody[i] >> IMGBITS & IMGMASK) - IMGMAX;
