...
chmod 740 ${N}N.chain && ./${N}N.chain
mv ${N}N_chain.data ../
#:<<BLOCK
for S in 2.0  #2.0 3.0 4.0 5.0 
do
	...
	b=$(echo "$D + 1.2" | bc)
	c=$(echo "$a + 0.8" | bc)
	sed "23s/5.0/${D}/;64s/30/${N}/;65,141s/0.1P/${Phi}P/;65,141s/1.0S/${S}S/;65,141s/2.0D/${D}D/;75s/1.2/$a/;75s/1.0/$b/g;84s/4.0/$c/;119s/100.0/${Box}/g" init.init > ${title}.init
    echo "lmp_wk..." 
	lmp_wk -i ${title}.init -l ${title}.log
    mv *.data ./src && mv ${title}.init ./src/ 
    mv ${title}u.lammpstrj ./src/ && mv ${title}.log ./src/
    cd src
done
#`BLOCK
