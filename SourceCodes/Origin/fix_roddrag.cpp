  ...
  Fix(lmp, narg, arg)
{
  ...
  double gamma_one = force->numeric(FLERR,arg[3]); //宽 
  double cw = force->numeric(FLERR,arg[4]); //长宽比 
...
void FixRoddrag::post_force(int vflag)
{
  ...
  for (int i = 0; i < nlocal; i++){
	if (mask[i] & groupbit) {
  	   if (mu[i][3] > 0.0) { //mu是单位矢量 
  	   	   double uvp[3],uve[3];  // uvp 平行分量，uve 竖直分量 
  	   	   double vp = v[i][0] * mu[i][0] + v[i][1] * mu[i][1] + v[i][2] * mu[i][2]; //平行投影大小 
  	   	   uvp[0] = mu[i][0] * vp; 
		   uvp[1] = mu[i][1] * vp; 
		   uvp[2] = mu[i][2] * vp; //平行分量 
  	   	   uve[0] = v[i][0] - uvp[0]; 
  	   	   uve[1] = v[i][1] - uvp[1];
  	   	   uve[2] = v[i][2] - uvp[2];  //竖直分量
  	   	   
           dragp = gamma[type[i]]; //平行方向摩擦系数 
		   drage = uuxy[type[i]];  //竖直方向摩擦系数 
		   
           f[i][0] -= dragp*uvp[0]+ drage*uve[0];  //最终黏滞力xyz坐标分量 
